...
<%
		...
		ArrayList<ComplainBean> nb = ud.getComplain(Integer.parseInt(request.getSession().getAttribute("userid").toString()));
		System.out.print(nb.isEmpty());
		if(nb.isEmpty())
		{
			response.sendRedirect("nocomplain");
		}
		...
		<table style="width:70%; margin-left:15%; margin-right:15%">
						...
						%>
								<tr>
									...
									<td class="column5"><%=nub.getComment()==null?"":nub.getComment()%></td>
									<td class="column6"><%out.print("<a href='deleteComplain/" + id +" '>"); %>
									<button class="contact100-form-btn" style=" min-width: 50px;margin-top: 5px;max-height: 40px; min-height: 25px; max-width: 150px;"  value="Delete" >Delete</button></td>
