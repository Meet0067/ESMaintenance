...
<%
       FundDao fd= (FundDao)request.getAttribute("fundDao");
		System.out.print(fd);
		int fId=Integer.parseInt(request.getParameter("fundId"));
		System.out.print(fId);
		
		//System.out.println(Integer.parseInt(request.getSession().getAttribute("userid").toString()));
		ArrayList<PaymentBean> pb = (ArrayList<PaymentBean>)fd.getPaidFunds(fId);
		if(pb.isEmpty())
		{
			
			response.sendRedirect("nopaid");
		}
%><br><br><br>
		<h2 align="center">Paid List For Fund ID <%=fId %></h2>
		<br><br><br>
		<div >
		<table style="width:70%; margin-left:15%; margin-right:15%">
						<thead>
							<tr class="table100-head">
								...
								<th class="column4">Flate Number </th>
								<th class="column4">Email</th>
								<th class="column4">Payment ID </th>
								<th class="column4">Payment Date</th>
						...
						<%
						for(PaymentBean pay :pb)
						{
							System.out.print(pay);
							//int id = (pay.getId());
						%>
								<tr>
									<td class="column2"><%= pay.getUserid() %></td>
									<td class="column3"><%= pay.getName() %></td>
									<td class="column4"><%= pay.getPhone_no() %></td>
									<td class="column4"><%= pay.getFlate_no() %></td>
									<td class="column4"><%= pay.getEmail()%></td>
									<td class="column4"><%= pay.getAuth_code() %></td>
									<td class="column4"><%= pay.getPayment_date()%></td>
								</tr>
